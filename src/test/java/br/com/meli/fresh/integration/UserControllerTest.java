...
public class UserControllerTest {
    ...
    @Test
    public void mustGetUserByID() throws Exception {
        User user = this.service.create(UserFactory.createUserA());
        UserResponseDTO responseDTO  = this.mapper.toResponseObject(user);
        MvcResult mvcResult =
                this.mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/fresh-products/users/{id}", user.getId())
    ...
    @Test
    public void mustPostUser() throws Exception {
        ...
        String jsonReturned =  mvcResult.getResponse().getContentAsString();
        UserResponseDTO userResponseDTO = new ObjectMapper().readValue(jsonReturned, UserResponseDTO.class);
        assertEquals(userResponseDTO.getEmail(), requestDto.getEmail());
    }

    @Test
    public void mustThrowEmailAlreadyExistsException() throws Exception {
        UserRequestDTO requestDto = UserFactory.createUserRequestDtoToThrow();
    ...
    public void mustUpdateUser() throws Exception {
        User user = this.service.create(UserFactory.createUserB());
        UserRequestDTO requestDto = UserFactory.createUserToUpdateRequestDto();
        ObjectWriter writer  = new ObjectMapper().configure(SerializationFeature.WRAP_ROOT_VALUE, false)
                .writer().withDefaultPrettyPrinter();
        String payloadRequestJson = writer.writeValueAsString(requestDto);

        MvcResult mvcResult = this.mockMvc.perform(MockMvcRequestBuilders.put("/api/v1/fresh-products/users/{id}", user.getId())
                .header(HttpHeaders.AUTHORIZATION, auth.token(mockMvc))
    ...
    @Test
    @Disabled
    public void mustGetAllUser() throws Exception {
    ...
    @Test
    public void mustDeleteUser() throws Exception {
        User user = this.service.create(UserFactory.createUserC());
        this.mockMvc.perform(MockMvcRequestBuilders.delete(BASE_URL + user.getId())
                        .header(HttpHeaders.AUTHORIZATION, auth.token(mockMvc)))
                .andDo(print()).andExpect(status().isOk()).andExpect(status().isOk()).andReturn();
        MvcResult mvcResult = this.mockMvc.perform(MockMvcRequestBuilders.get(BASE_URL + user.getId())
                        .header(HttpHeaders.AUTHORIZATION, auth.token(mockMvc)))
                .andDo(print()).andExpect(status().isNotFound()).andReturn();
